...
#include "dali/operators/reader/loader/file_loader_new.h"
...
void FileLoader::ReadSample(ImageLabelWrapper &image_label) {
  ...
  //std::cout << "Reading Current index = " << cur_idx << ", img = " << image_pair.first << std::endl;
  ...
  /*
   * We need to do is update the path of the file to point to the memory area instead of disk
   * If cache size is not full, then add this entry to cache,else do nothing
  */
  //std::cout << "SHM cache list length " << shm_cache_index_list_.size() << endl;
  if (cache_size_ > 0 && !caching_done_){
      ...
      //std::cout << "Searching for " << image_pair.first << " found : " << must_cache << " cache done? : " << caching_done_ << endl; 
      if (!caching_done_ && must_cache) {
        RawDataDache::CacheRawDataEntry *ce = new shm::CachRawDataeEntry(image_pair.first);
        int ret = -1;
  ...
  if (cache_size_ > 0 && caching_done_){
	...
	if (is_cached(image_pair.first)){
    		...
    		//std::cout << "Got local cached value for " << image_pair.first << endl;
	}
	//Check other nodes
	else if ((node = shm::is_cached_in_other_node(shm_cache_index_list_other_nodes, image_pair.first, node_id_)) >= 0) {
                //std::cout << "Available in node " << node << " for " << image_pair.first << " on " << server_fd_[node] << endl;
		use_prefix = false;
      		string fn = file_root_ + "/" + image_pair.first;
      		long long image_size = getFilesize(fn.c_str());
	    	//std::cout << "\tImage size " << image_size << " for " << image_pair.first << endl;
		if (image_size < 0) {
			//std::cout << "Unsuccessful header read from node " << node << " for " << image_pair.first << endl;
			prefix = file_root_;
			use_prefix = true;
		}
		else { 
			...
			image_label.image.Resize({image_size});
        		net_mutex_.lock();
			int bytes_read = shm::ReadFromOtherNode(rank[node], image_pair.first, image_label.image.mutable_data<uint8_t>(), image_size);
        
        		net_mutex_.unlock();		
			if (bytes_read < 0){
				//std::cout << "Unsuccessful read from node " << node << " for " << image_pair.first << endl;
				prefix = file_root_;
				use_prefix = true;
			}
			else{
    				//std::cout << "Got remote cached value for " << image_pair.first << endl;
			}
  		}
	}
        else
		std::cout << "Sample " << image_pair.first << " not found in any cache. Must fetch" << endl;
  }
  else
    prefix = file_root_;
