...
#ifndef DALI_OPERATORS_READER_LOADER_FILE_LOADER_NEW_H_
#define DALI_OPERATORS_READER_LOADER_FILE_LOADER_NEW_H_
...
#include "dali/operators/shmcache/raw_data_cache.h"
...
class FileLoader : public Loader<CPUBackend, ImageLabelWrapper> {
 public:
  ...
  ~FileLoader() {
      ...
      //}
     //std::cout << "Order of shm cached items : " << endl;
     //for(int i=0; i < static_cast<int>(shm_cached_items_.size()); i++)
         //std::cout << i << " : " << shm_cached_items_[i] << endl;
   }
 ...
 protected:
  ...
  void PrepareMetadataImpl() override {
    ...
    //std::cout << "Per shard = " << size_per_shard << ", cache = " << cache_size_ << ", extra cache = " << extra_cache_size_ << ", orig cache = "<<cache_size_orig_ << endl;
  ...
  void Reset(bool wrap_to_shard) override {
    if (wrap_to_shard) {
      ...
      //std::cout << "Current Epoch : " << current_epoch_ << endl;
      //std::cout << "Cache size : " << cache_size_  << "\nCached : " << caching_done_ << endl;
    ...
    if (current_epoch_ == 1){
       ...
       if ( num_nodes_ > 1 && cache_size_ > 0 && !resume_) { 
           ...
           if (extra_cache_size_ > 0 && dist_mint){
               ...
               //std::cout << "Items not cached in current node = " << items_not_in_node.size() << endl;
               Index items_per_shard = items_not_in_node.size()/num_shards_per_node_;
               //std::cout << "Extra items required per shard = " << extra_cache_size_ << ", available indexes per shard " << items_per_shard << endl;
               ...
               //std::cout << "Shard " << shard_id_ << " start : " << start_idx << ", end : " << end_idx << endl;
               mint_prefetcher = std::thread(shm::PrefetchRawData, items_not_in_node, start_idx, end_idx, file_root_);
    ...
    if (!caching_done_ && cache_size_ > 0){
       //std::cout << "Seed is " << shuffle_seed_ << endl;
       ...
       if ( num_nodes_ > 1) {
           ...
           for (int nid = 0; nid < num_nodes_; nid ++ ){
               ...
               for (int sh = 0; sh < num_shards_per_node_; sh ++){
                   ...
                   //std::cout << "\tShard " << nid*num_shards_per_node_ + sh << ", size " << shard_size << ", nid " << nid <<  std::endl;
                   //std::cout << "\t\t Index begin " << shard_start_idx << ", index end " << shard_end_idx << std::endl;
                   ...
                   //std::cout << "For node " << nid <<  " shard : " << sh << endl;
		     ...
		     //std::cout << "\t" << k << " : " << cache_list_per_shard_name[k] << std::endl; 
                   }
	   ...
	   std::cout << "For node " << nid << endl;
               for (int i = 0; i < static_cast<int>(shm_cache_index_list_other_nodes[nid].size()); i++) 
	       std::cout << "\t" << i << " : " << shm_cache_index_list_other_nodes[nid][i] << std::endl;
           }
       }*/
       ...
       std::cout << "Index list to cache for this shard : " << endl;
       for (int i = 0; i < static_cast<int>(shm_cache_index_list_.size()); i++)
          //std::cout << i << " : " << shm_cache_index_list_[i]  << " : " << shm_cache_name_list_[i] << std::endl;
    }
...
#endif  // DALI_OPERATORS_READER_LOADER_FILE_LOADER_NEW_H_
